...
def second_step(message):
    ...
    global new_markup
    global current_collection_name 
    if message.text in list(data['Коллекция']):
        current_collection_name = message.text
    current_catal = []
    new_markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    for i in range(len(list(data[data['Коллекция'] == current_collection_name]['название']))):
        current_good = list(data[data['Коллекция'] == current_collection_name]['название'])[i]
        ...
        new_markup.add(types.KeyboardButton(list(data[data['Коллекция'] == current_collection_name]['название'])[i]))
    new_markup.add(types.KeyboardButton("Назад"))
    current_catal = list(data[data['Коллекция'] == current_collection_name]['название'])
...
def fourth_step(message):
    ...
    if int(message.text) <= list(data[data['название'] == current_good]['Наличие'])[0]:
        ...
        bot.send_message(message.chat.id, text="Столько товара нету в наличии. Введите допустимое значение:", reply_markup=types.ReplyKeyboardRemove)
        current_step = 4
